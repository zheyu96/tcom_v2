#ifndef __EFIRAP_LONGTIME_H
#define __EFIRAP_LONGTIME_H

// WernerAlgo*.h defines `double` as a preprocessor macro and does not restore
// it. Isolate EFiRAP_longtime's public ABI from that leaking macro so its
// constructor has the same signature regardless of include order.
#ifdef double
#pragma push_macro("double")
#undef double
#define EFIRAP_LONGTIME_RESTORE_DOUBLE_MACRO
#endif
 ...
 */
class EFiRAP_longtime : public AlgorithmBase {
public:
    ...
    // True only when EFiRAP_longtime.cpp is compiled with -DEFIRAP_USE_GUROBI.
...
#ifdef EFIRAP_LONGTIME_RESTORE_DOUBLE_MACRO
#pragma pop_macro("double")
#undef EFIRAP_LONGTIME_RESTORE_DOUBLE_MACRO
#endif
